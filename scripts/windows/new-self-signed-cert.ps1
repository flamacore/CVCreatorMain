param(
  [string]$Subject = "CN=CVCreator Self Signed",
  [string]$FriendlyName = "CVCreator Self Signed",
  [int]$ValidYears = 3,
  [string]$ExportPath = ".\certificates\cvcreator-self-signed.pfx",
  [string]$Password = ""
)
...
if (-not $Password.Trim() -and $Subject -and -not $Subject.StartsWith("CN=")) {
  $Password = $Subject
  $Subject = "CN=CVCreator Self Signed"
}
