...
function Resolve-CodeSigningCertificateThumbprint {
  ...
  if ($env:CVCREATOR_CERT_THUMBPRINT) {
    return $env:CVCREATOR_CERT_THUMBPRINT.Trim()
  }
...
$requireSigning = $env:CVCREATOR_REQUIRE_SIGNING -eq "1"
$subject = if ($env:CVCREATOR_SIGN_SUBJECT) { $env:CVCREATOR_SIGN_SUBJECT } else { "CN=CVCreator Self Signed" }
$thumbprint = Resolve-CodeSigningCertificateThumbprint -Subject $subject

if (-not $thumbprint) {
  if ($requireSigning) {
    throw "No signing certificate was found. Generate one with 'npm run release:windows:cert' or set CVCREATOR_CERT_THUMBPRINT."
  }
...
if ($env:CVCREATOR_TIMESTAMP_URL) {
  $arguments += @(
    "/tr",
    $env:CVCREATOR_TIMESTAMP_URL,
    "/td",
    "SHA256"
  )
}
